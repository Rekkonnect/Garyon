...
/// <summary>
/// Represents a set whose elements are stored in a queue. This data structure
/// encapsulates a <seealso cref="Queue{T}"/> and a
/// <seealso cref="HashSet{T}"/>. Through the usage of the set, the data
...
public sealed class QueueSet<T> : BaseSetLinearCollection<T>
{
    ...
    /// <summary>
    /// Initializes a new empty instance of the <seealso cref="QueueSet{T}"/>
    /// class.
    /// </summary>
    public QueueSet()
        : base()
    ...
    /// <summary>
    /// Initializes a new empty instance of the <seealso cref="QueueSet{T}"/>
    /// class with a given comparer for the <seealso cref="HashSet{T}"/>.
    /// </summary>
    /// <param name="comparer">
    ...
    /// queue set and was successfully enqueued, otherwise
    /// <see langword="false"/>.
    /// </returns>
    ...
    /// <summary>
    /// Dequeues the item at the start of the queue, and also removes it from
    /// the set.
    /// </summary>
    /// <returns>
    /// The dequeued item.
    ...
    /// <summary>
    /// Enqueues a collection of items to the queue set. The order at which each
    /// item is enqueued is determined by the enumeration order of the provided
    /// collection.
    /// </summary>
    /// <param name="items">
    /// The collection of items to enqueue to the queue set.
    /// </param>
    ...
    /// <summary>
    /// Dequeues a number of items from the queue set, and returns them in the
    /// order they were dequeued.
    /// </summary>
    /// <param name="count">
    /// The number of items to dequeue from the queue set. If the queue set
    /// contains less items than the provided count, it will be capped to the
    /// current item count.
    /// </param>
    /// <returns>
    ...
    /// <summary>
    /// Gets the enumerator for this queue set, which is the enumerator for the
    /// queue.
    /// </summary>
    /// <returns>
